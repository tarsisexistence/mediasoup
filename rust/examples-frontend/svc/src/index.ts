...
async function init() {
    const sendPreviewNode = document.querySelector('#preview-send') as HTMLVideoElement;
    const receivePreviewNode = document.querySelector('#preview-receive') as HTMLVideoElement;

    sendPreviewNode.onloadedmetadata = () => sendPreviewNode.play();
    receivePreviewNode.onloadedmetadata = () => receivePreviewNode.play();
    ...
    ws.onmessage = async (message) => {
        ...
        switch (decodedMessage.action) {
            case 'Init': {
                ...
                sendPreviewNode.srcObject = mediaStream;
                ...
                // Consumer transport is now needed to receive previously produced tracks back
                ...
                for (const producer of producers) {
                    await new Promise((resolve) => {
                        ...
                        waitingForResponse.set('Consumed', async (consumerOptions: ConsumerOptions) => {
                            // Once confirmation is received, corresponding consumer can be created client-side
                            ...
                            receivePreviewNode.srcObject = receiveMediaStream;
