...
impl SvcConnection {
    ...
    async fn new(worker_manager: &WorkerManager) -> Result<Self, String> {
        ...
        // We know that for svc example we'll need 2 transports, so we can create both right away.
...
impl StreamHandler<Result<ws::Message, ws::ProtocolError>> for SvcConnection {
...
impl Handler<ClientMessage> for SvcConnection {
    type Result = ();
...
impl Handler<ServerMessage> for SvcConnection {
    type Result = ();
...
impl Handler<InternalMessage> for SvcConnection {
    type Result = ();
...
async fn ws_index(
    ...
    match SvcConnection::new(&worker_manager).await {
        Ok(svc_server) => ws::start(svc_server, &request, stream),
        Err(error) => {
            eprintln!("{}", error);
